    ...
    if (data?.confirmedInput?.userId) {
      ...
      try {
        const res = await fetch('/api/roblox/games', {
          ...
          body: JSON.stringify({ userId: data.confirmedInput.userId })
        });
...
<div class="flex flex-col gap-4">
  ...
  <div class="w-full h-[3px] bg-[#1D2535] rounded-full"></div>
  ...
  {:else}
    <div class="p-6 text-center text-[#809BB5]">
