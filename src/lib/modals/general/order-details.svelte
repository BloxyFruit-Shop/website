            ...
            {#each automaticItems as item}
              <div class="p-3 bg-[#1A1F2E] rounded-lg">
                ...
                {#if item.inventoryItems?.length > 0}
                  <div class="mt-4 p-3 bg-[#1D2535] rounded-lg flex flex-col gap-2">
                    {#each item.inventoryItems as account}
                      <div class="flex">
                        <div class="bg-[#131620] flex items-center gap-2 py-2 px-3 rounded-l-lg flex-1">
                          <User class="size-5" />
                          <p class="text-sm font-medium">{account.data.username}</p>
                        </div>
                        <div class="bg-[#131620] flex items-center gap-2 py-2 px-3 border-l-[3px] border-[#1D2535] rounded-r-lg flex-1">
                          <Password class="size-5" />
                          <p class="text-sm font-medium">{account.data.password}</p>
                        </div>
                      </div>
                    {/each}
                  </div>
