<script>
  ...
  import {
    ...
    formatCurrency,
    formatNumber,
    currencies as currenciesObject,
  ...
  onMount(() => {
    if (window.matchMedia('(max-width: 768px)').matches) {
      const observer = new MutationObserver(() => {
        const initButton = document.querySelector('[aria-label="Open chat"]');
        if (initButton) {
          initButton.style.setProperty('bottom', '80px', 'important');
          observer.disconnect();
        }
        const chatButton = document.querySelector('#crisp-chatbox-button');
        if (chatButton) {
          chatButton.style.setProperty('bottom', '80px', 'important');
          observer.disconnect();
        }
      });
    ...
    if (
      ...
      data?.game?.internalName === $cart?.game
    )
      return;

    fetch(
      `/api/products?game=${$cart.game}&variantIds=${$cart.products.map((item) => item.variantId.replace('gid://shopify/ProductVariant/', '')).join(',')}`
    )
  ...
  $: if (Array.isArray($cart.products)) {
    cartProducts = $cart.products.map((cartItem) => {
      ...
      if (data.game.internalName === $cart.game) {
        product = data.game.products.find(
          (p) => p.variantId === cartItem.variantId
        );
      } else {
        ...
        product = existingProduct
          ? existingProduct
          : { title: 'Unknown', image: '', price: 0, inStock: false };
      }
  ...
  $: categorizedProducts = categorizeProducts(
    data.game.products,
    data.game.order
  );

  let cartIsAtTop = true,
    cartIsAtBottom = false;
  ...
  const handleCategorySelector = () => {
    ...
    if (isAtBottom) {
      $activeCategory = categories.findLast(
        (category) => filteredCategories[category]?.length > 0
      );
      return;
    }
...
<svelte:head>
  <title
    >BloxyFruit - {data.game.name} Items with Cheap Prices & Fast Delivery!</title
  >
  <meta
    property="og:title"
    content="BloxyFruit - {data.game
      .name} Items with Cheap Prices & Fast Delivery!"
  />
  <meta
  ...
  <meta
    name="description"
    content="Discover unbeatable deals on {data.game
      .name} items at BloxyFruit! From the cheapest {data.game
      .name} items to rare Anime Vanguards, Rivals, and Pets Go items, we offer cheap prices, fast delivery, and trusted service. Get your dream items at the best prices today!"
  />
  <meta
  ...
  <meta
    name="keywords"
    content="blox fruits, cheap blox fruits, blox fruits items, buy blox fruits, fast blox fruits delivery, anime defenders, anime defenders items, cheap anime defenders, buy anime defenders, the strongest battlegrounds, battlegrounds gamepasses, cheap gamepasses, Roblox items, buy Roblox items, fast delivery Roblox, online Roblox shop, game items, cheap game items, Roblox gamepasses, buy gamepasses, murder mystery 2, anime vanguards, adopt me"
  />
</svelte:head>

<img
  src="/assets/background.png"
  class="absolute z-20 pointer-events-none select-none size-full"
/>
...
<MobileNavbar>
  ...
  >
    <ShoppingCart
      class="size-6 transition-colors group-data-[selected=true]:text-[#3BA4F0]"
    />
    <p
      class="font-semibold leading-none transition-colors group-data-[selected=true]:text-white"
    >
      Cart
    </p>
    <p
...
>
  <div class="flex justify-end flex-1 gap-3">
    ...
    <p class="text-sm font-semibold">
      Spend {formatCurrency(100 * $currencyRateStore, $currencyStore)} — Get
      <span class="text-[#3BA4F0]"
        >{formatCurrency(10 * $currencyRateStore, $currencyStore)}</span
      > Off!
    </p>
  </div>
  <div class="w-[300px] h-2.5 bg-[#1D2535] rounded-xl">
    <div
      class="bg-gradient-to-r from-[#3BA4F0] to-[#1B92E9] rounded-full h-2.5 origin-left transition-[width] ease-out animate-scale-x"
      style="width: {Math.min((cartPrice / 100) * 100, 100)}%"
    ></div>
  </div>
  <div class="flex justify-start flex-1 gap-3">
    {#if cartPrice < 100}
      <p class="text-sm font-semibold">
        Add <span class="bg-[#5CB5F6]/25 text-[#C8E8FF] py-1 px-1.5 rounded-md"
          >{formatCurrency(
            (100 - cartPrice) * $currencyRateStore,
            $currencyStore
          )}</span
        > more to unlock discount!
      </p>
    {:else}
...
<header
  class="px-4 sm:px-6 md:px-10 flex justify-center items-center w-full h-[80px] bg-[#121620] border-b-2 border-[#1D2535]"
>
  <div class="flex items-center justify-between w-full">
    ...
    <div class="flex flex-1 justify-end items-center gap-4 @container">
      ...
      >
        <div
          class="max-md:hidden flex items-center gap-2 bg-[#1D2535] relative h-[46px] rounded-lg px-2.5 focus-within:ring-2 focus-within:ring-[#3BA4F0] transition-shadow {searchExpanded
            ? 'w-[240px]'
            : 'w-[46px]'} @[760px]:w-[240px]"
        >
        ...
        >
          <div
            class="flex flex-col gap-1 [&::-webkit-scrollbar-thumb]:bg-[#809BB5] overflow-y-auto"
          >
                  ...
                  >
                    <div class="flex items-center flex-1">
                      <div class="size-10 p-1.5 mr-2 bg-white/5 rounded-md">
                        {#if i > 15}
                          <img
                            use:lazyLoad={{ src: result.image }}
                            class="object-contain object-center rounded-md opacity-0 size-full"
                          />
                        {:else}
                          <img
                            src={result.image}
                            class="object-contain object-center rounded-md size-full"
                          />
                        {/if}
                      </div>
                      <div class="flex flex-col gap-1">
                        ...
                        <p
                          class="text-[13px] text-[#809BB5] font-medium leading-none"
                        >
                          {result.category}
                        </p>
                      </div>
      ...
      <div
        class="items-center gap-4 flex {searchExpanded
          ? 'hidden @[760px]:flex'
          : ''}"
      >
        <div class="hidden gap-1 md:flex">
          <div
            class="bg-[#1D2535] rounded-l-lg flex items-center px-3 font-semibold text-sm"
          >
            <p class="text-[#809BB5] mr-2">
              ({$cart?.products?.length || 0} items)
            </p>
            <p>
              {formatCurrency(cartPrice * $currencyRateStore, $currencyStore)}
            </p>
          </div>
          <Button
            color="accent"
            ...
            onClick={() => {
              desktopCartOpen = !desktopCartOpen;
            }}
          >
        ...
        <LocaleSelector
          initialLanguage={data.lang}
          initialCurrency={data.curr}
        />
        <UserSection {data} />
      </div>
    </div>
...
<header
  class="px-4 sm:px-6 md:px-10 flex justify-center items-center w-full h-[80px] bg-[#121620] border-b-2 border-[#1D2535] md:hidden"
>
  <div class="flex items-center w-full gap-4">
    <Select.Root items={games} bind:selected={game}>
      ...
      >
        <div class="pointer-events-none size-6 md:size-7">
          <img
            src="/assets/games/{games[game.value].internalName}-logo.webp"
            class="object-contain object-center rounded-md size-full"
          />
        </div>
        <p class="ml-2 font-semibold pointer-events-none">
          {games[game.value].shortName}
        </p>
        <ArrowDown
        ...
        >
          {#each Object.values(games) as inGame}
            ...
            >
              <a
                href={`/store/${inGame.internalName}`}
                class="flex items-center justify-center w-full py-1.5 px-2.5"
              >
                <div class="size-6 mr-1.5">
                  <img
                    src="/assets/games/{inGame.internalName}-logo.webp"
                    alt="Game Icon"
                    class="object-contain object-center rounded-md size-full"
                  />
                </div>
    ...
    <div
      class="bg-[#1D2535] rounded-lg flex items-center px-3 h-[46px] flex-1 min-w-0"
    >
      <Search class="size-5 text-[#809BB5] shrink-0" />
      <input
        type="text"
        class="bg-transparent outline-none text-white px-2.5 py-2.5 placeholder:text-[#809BB5] font-medium flex-1 min-w-0"
...
<div class="flex h-[calc(100dvh-232px)] md:h-[calc(100dvh-124px)]">
  ...
  >
    <div class="flex items-center mb-4 md:hidden">
      ...
      <Button
        ...
        variant="text"
        color="white"
        wrapperClass="md:hidden ml-auto"
    ...
    <Accordion.Root multiple value={['$0', '$1', '$2']}>
        ...
        >
          {#each Object.values(games) as game, i}
            <Button
              variant="bordered"
              color="game"
              to={`/store/${game.internalName}`}
              wrapperClass="w-full"
              class="w-full {$page.url.pathname == `/store/${game.internalName}`
                ? 'bg-[#1D2535] border-[#1D2535]'
                : 'bg-[#1D2535]/25 border-[#1D2535]/55'}"
              insideDivClass="w-full justify-between"
            >
              <div class="size-7 mr-1.5">
                <img
                  src="/assets/games/{game.internalName}-logo.webp"
                  class="object-contain object-center rounded-lg size-full"
                />
              </div>
              {game.name}
              <div
                class="rounded-lg size-7 flex items-center justify-center ml-auto {$page
                  .url.pathname == `/store/${game.internalName}`
                  ? 'bg-[#3BA4F0]'
                  : 'bg-[#1D2535]'}"
              >
      ...
      <Accordion.Item
        value="$1"
        class="bg-[#141925] border-2 border-[#1D2535] rounded-lg"
      >
        <Accordion.Header>
          <Accordion.Trigger
        ...
        >
          ...
          <div class="flex items-center justify-between gap-2 mt-2">
            <div
              class="flex items-center w-full bg-[#1D2535] py-2 px-2.5 gap-1.5 rounded-md"
            >
              <p class="font-semibold">{currency.symbol}</p>
              <p class="flex-1 font-medium text-right">
                {formatNumber(priceRange[0] * $currencyRateStore)}
              </p>
            </div>
            ...
            >
              <p class="font-semibold">{currency.symbol}</p>
              <p class="flex-1 font-medium text-right">
                {formatNumber(priceRange[1] * $currencyRateStore)}
              </p>
            </div>
          </div>
        </Accordion.Content>
      ...
      <Accordion.Item
        value="$2"
        class="bg-[#141925] border-2 border-[#1D2535] p-3.5 rounded-lg mt-3.5"
      >
        <Accordion.Header>
          <Accordion.Trigger
          ...
          {#each rarities as rarity, i}
            <div class="flex items-center justify-between first:mt-3.5">
              <div class="flex items-center">
                <Tag
                  class="size-5"
                  style="color: {rarityColors[rarity.color]
                    .solid}; filter: drop-shadow(0px 0px 5px {alpha(
                    rarityColors[rarity.color].solid,
                    0.3
                  )})"
                />
                <p class="font-semibold ml-1.5">
                  {rarity.name} -
                  <span class="text-[#809BB5] text-xs"
                    >{allItems.filter((item) => item.rarity === rarity.name)
                      .length}</span
                  >
                </p>
              </div>
              <button
                class="rounded-lg relative size-6 flex items-center justify-center ml-auto overflow-hidden bg-[#1D2535]"
                on:click={() => (rarities[i].enabled = !rarities[i].enabled)}
              >
                <div
                  class="bg-[#3BA4F0] size-full rounded-lg transition-transform ease-in-out absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 will-change-transform {rarity.enabled
                    ? 'scale-100'
                    : 'scale-0'}"
                ></div>
                <div
                  class="rounded-full size-2 bg-white transition-opacity z-10 {rarity.enabled
                    ? 'opacity-100'
                    : 'opacity-0'}"
                ></div>
              </button>
  ...
  <div class="relative flex flex-col flex-1 overflow-hidden">
    <div class="p-6">
      <div
        class="bg-[#1D2535]/35 rounded-lg gap-5 flex items-center overflow-x-auto"
      >
        <div
          class="relative flex items-center flex-1 gap-5 p-3 md:p-4"
          style="min-width: {categories.reduce(
            (acc, item) => acc + item.length * 9.5 + 25,
            200 + 50 + 25 + 25
          )}px"
        >
          <div
            class="opacity-0 w-16 h-1 rounded-t-xl absolute bg-[#3BA4F0] bottom-0 shadow-[0_-5px_28px_#3BA4F0] transition-[transform,width,opacity]"
          ...
          {#each categories as category}
            <button
              ...
              id="{category}-tab"
              data-enabled={category == $activeCategory &&
            ...
            >
              {#if category == 'Best Sellers'}
                <Fire
                  class="group-data-[enabled=true]:text-[#3BA4F0] size-6 mr-1 transition-colors"
                />
              {/if}
              {pluralize(category)}
            </button>
          ...
          <div class="flex gap-1 ml-auto">
            <div
              class="bg-[#1D2535] rounded-l-lg text-[#809BB5] flex items-center p-2.5"
            >
    ...
    <div
      ...
      class="flex flex-col flex-1 px-6 pb-6 overflow-y-auto"
      on:scroll={handleCategorySelector}
    >
      {#each categories as category}
        ...
        {#if products?.length > 0}
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
              ...
              {/if}
              <p class="py-4 text-xl font-semibold" id="{category}-category">
          ...
          <div
            class="grid [grid-template-columns:repeat(auto-fill,minmax(150px,1fr))] sm:[grid-template-columns:repeat(auto-fill,minmax(210px,1fr))] gap-6"
          >
            {#each products as { title, image, price, comparePrice, variantId, inStock, category, description, askBeforeBuy, rarity }}
              <WholeItem
                ...
                onClick={() => {
                  if (
                    $cart.game &&
                    $cart?.game !== data.game.internalName &&
                    $cart?.products?.length > 0
                  ) {
                  ...
                  if (
                    $cart.game == 'blox-fruits' &&
                    $cart?.products?.length > 0 &&
                    $cart?.products?.some(
                      (item) =>
                        item.category === 'Physical Fruit' ||
                        category === 'Physical Fruit'
                    )
                  ) {
  ...
  >
    ...
    <div
      class="pointer-events-none absolute top-[79px] left-0 right-1.5 bg-gradient-to-b from-[#121620] to-transparent z-10 transition-[opacity,height] {cartIsAtTop
        ? 'opacity-0 h-0'
        : 'opacity-100 h-12'}"
    ></div>
    <div
      class="pointer-events-none absolute bottom-[335px] md:bottom-[275px] left-0 right-1.5 bg-gradient-to-b from-transparent to-[#121620] z-10 transition-[opacity,height] {cartIsAtBottom
        ? 'opacity-0 h-0'
        : 'opacity-100 h-12'}"
    ></div>
    ...
    <div class="relative flex-1 py-4 overflow-hidden">
      {#if cartProducts?.length == 0}
        <div
          class="absolute flex flex-col items-center justify-center w-full gap-1 -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
          in:fly={{ y: 20 }}
        ...
        >
          <div class="relative">
            ...
            <p
              class="absolute top-0 right-0 size-5 text-sm rounded-full bg-[#3BA4F0] font-semibold flex items-center justify-center"
            >
              0
            </p>
          </div>
          <p class="mt-2 text-xl font-bold leading-none text-center">
            Your cart is empty
          </p>
          <p
            class="text-center text-[15px] font-medium text-[#809BB5] leading-none mt-0.5"
          >
            Start browsing for items now!
          </p>
        </div>
      {/if}

      <div
        class="relative flex flex-col h-full space-y-3 overflow-y-auto no-scrollbar"
        on:scroll={(e) => {
          (cartIsAtTop = e.target.scrollTop === 0),
            (cartIsAtBottom =
              Math.abs(
    ...
    <div
      class="mt-auto flex flex-col p-2.5 bg-[#141925] border-2 border-[#1D2535] rounded-lg"
    >
      {#if cartPrice < 100}
        <p class="font-semibold">
          Spend <span class="text-[#3BA4F0]"
            >{formatCurrency(100 * $currencyRateStore, $currencyStore)}</span
          >, Get
          <span class="text-[#3BA4F0]"
            >{formatCurrency(10 * $currencyRateStore, $currencyStore)}</span
          > Off!
      ...
      {:else}
        <div class="flex items-center gap-1.5">
          <Check
            class="size-[18px] p-[3px] bg-[#3BA4F0] rounded-full"
            strokeWidth="4"
          />
      ...
      <div class="flex items-center gap-2 mt-2">
        ...
        <div class="flex-1 h-2.5 bg-[#1D2535] rounded-xl">
          <div
            class="bg-gradient-to-r from-[#3BA4F0] to-[#1B92E9] rounded-full h-2.5 origin-left animate-scale-x transition-[width]"
            style="width: {Math.min((cartPrice / 100) * 100, 100)}%"
          ></div>
        </div>
    ...
    <div class="mt-1 text-sm text-[#809BB5] font-semibold text-center">
      ...
      {:else}
        <button
          ...
          class="text-[#809BB5] hover:text-[#5bc0ff] flex items-center justify-center gap-1 w-full"
        >
          <span
            >{currentRef
              ? `Referral Code: ${currentRef}`
              : 'Add Referral Code'}</span
          >
    ...
    <div class="flex justify-center gap-2 mt-3">
      {#each [Mastercard, Maestro, Paypal, Visa] as icon}
        <div
          class="bg-white rounded-lg w-[54px] h-[32px] flex items-center justify-center"
        >
...
<Modal bind:open={permanentFruitPresent}>
  ...
  <p class="mt-6 font-semibold">
    <span class="text-[#3BA4F0]">Physical Fruits</span> can't be combined with
    other items.
    <br />
    Please remove all <span class="text-[#3BA4F0]">Physical Fruits</span> or clear
    your cart to add this item.
  </p>
  <Button
    variant="gradient"
    color="accent"
...
<Modal bind:open={multipleGamesOpen}>
  <h1 class="text-xl font-semibold">Oops! Cart Update Needed</h1>
  <p class="mt-6 font-semibold">
    Your cart currently contains items from <span class="text-[#3BA4F0]"
      >{games[$cart.game].name}</span
    >. <br />
    To add items from <span class="text-[#3BA4F0]">{data.game.name},</span> please
    remove any items from other games.
  </p>
  <Button
    variant="gradient"
    color="accent"
...
<Modal bind:open={askBeforeBuyModal} contentClass="min-[870px]:max-w-[740px]">
  <div class="flex gap-6">
    <div
      ...
      style="background: {rarityColors[
        games?.[data.game.internalName]?.rarities?.find(
          (r) => r.name === askBeforeBuyModal.rarity
        )?.color || 'gray'
      ].gradient}"
    >
      <CirlceIlustration
        class="absolute bottom-[-308px] right-[-308px] rotate-90 size-[524px]"
        color={rarityColors[
          games?.[data.game.internalName]?.rarities?.find(
            (r) => r.name === askBeforeBuyModal.rarity
          )?.color || 'gray'
      ...
      <div class="relative pointer-events-none select-none size-full">
        <img
          use:lazyLoad={{ src: askBeforeBuyModal.image, opacity: 0.2 }}
          alt="Item thumbnail"
          class="size-full object-contain object-center absolute top-0 left-0 blur-xl z-0 opacity-35 transition-[opacity,transform] duration-200"
        />
        <img
          use:lazyLoad={{ src: askBeforeBuyModal.image }}
          alt="Item thumbnail"
          class="relative z-10 object-contain object-center transition-opacity opacity-0 size-full"
        />
      </div>
    </div>
    <div class="flex flex-col gap-1 py-2">
      <h1 class="text-3xl font-semibold leading-none">
        {askBeforeBuyModal.title}
      </h1>
      <p class="text-sm text-[#809BB5] font-semibold leading-none">
        {askBeforeBuyModal.category}
      </p>
      <div class="flex max-sm:flex-col sm:items-center gap-1 !leading-none">
        <p class="text-[17px] text-left font-extrabold text-white">
          {formatCurrency(
            askBeforeBuyModal.price * $currencyRateStore,
            $currencyStore
          )}
        </p>
        {#if askBeforeBuyModal.comparePrice}
          <p
            class="text-sm font-semibold text-[#C42F30] text-left line-through"
          >
            {formatCurrency(
              askBeforeBuyModal.comparePrice * $currencyRateStore,
              $currencyStore
            )}
          </p>
        {/if}
      ...
      <div
        class="text-[#809BB5] font-semibold leading-none whitespace-pre-line [word-wrap:break-word] mt-3"
      >
        {@html askBeforeBuyModal.description}
      </div>
      <Button
        variant="gradient"
        color="accent"
        ...
        onClick={() => {
          let newProducts = $cart?.products || [];
          const itemIndex = newProducts?.findIndex(
            (product) => product.variantId == askBeforeBuyModal.variantId
          );
...
<style>
  :global(.crisp-client) {
    --crisp-customization-mobile-button-vertical: 63px !important;
  }
</style>