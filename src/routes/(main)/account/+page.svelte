<script>
  ...
  function openOrderDetails(order) {
    if (!order?.reciever?.username && order.status === 'pending') {
      toast['error']('You need to add a reciever before claiming your order.', {
        duration: 3_000
      });
      return;
    }
...
<svelte:head>
  <title>Account - BloxyFruit</title>
  <link rel="canonical" href="https://bloxyfruit.com/account" />
</svelte:head>
...
<div class="px-6 min-h-[calc(100dvh-178px-104px)] pb-10">
  ...
  <div
    class="max-w-[1440px] w-full mx-auto mt-[104px] flex flex-col lg:flex-row gap-6"
  >
    <div class="flex flex-col w-full gap-6 lg:w-4/12 h-fit">
      ...
      <div
        class="w-full p-6 pb-2 rounded-lg"
        style={bgBlur({ color: '#111A28', blur: 6, opacity: 0.9 })}
      >
        <div class="flex items-center justify-between gap-2">
          <p class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2 text-lg font-medium">
              ...
              <div class="flex items-center gap-1">
                {#if data.referral}
                  <ClipboardCopy
                    class="underline"
                    copy={`https://bloxyfruit.com/?ref=${data.referral}`}
                    successMessage="Referral link copied to clipboard"
                    >bloxyfruit.com/?ref={data.referral}
                    <Link class="size-3" /></ClipboardCopy
                  >
                {:else}
                  <p class="text-sm text-gray-400">
                    Not available. Try logging in again.
                  </p>
                {/if}
              </div>
            </div>
              ...
              <div class="flex items-center gap-1">
                {#if data.referral}
                  <ClipboardCopy
                    ...
                    >{data.referral}<Link class="size-3" /></ClipboardCopy
                  >
                {:else}
                  <p class="text-sm text-gray-400">
                    Not available. Try logging in again.
        ...
        <a
          ...
          target="_blank"
          class="flex items-center justify-center w-fit mx-auto my-2 gap-2 p-2 text-[#809BB5] hover:text-white transition-colors rounded-lg"
        >
      ...
      >
        <Collapsible.Root>
          <Collapsible.Trigger
            class="w-full flex items-center justify-between p-2 rounded-lg hover:bg-[#1D2535]/30 transition-colors"
          >
            <div class="flex items-center gap-2">
              <p class="text-[#809BB5]">View Claims</p>
              {#if data.claims?.length}
                <span class="text-sm text-[#809BB5]"
                  >({data.claims.length})</span
                >
              {/if}
            </div>
            <ArrowDown
              class="size-4 text-[#809BB5] transition-transform duration-200 collapsible-icon"
            />
          </Collapsible.Trigger>
          ...
          <Collapsible.Content class="pt-2">
            ...
            >
              ...
              {:else}
                <p class="text-sm text-[#809BB5] px-2">
    ...
    <div
      class="flex flex-col flex-shrink-0 w-full p-6 rounded-lg lg:w-8/12 h-fit"
      style={bgBlur({ color: '#111A28', blur: 6, opacity: 0.9 })}
    >
      <div class="shrink-0">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          {#if data.pagination && data.pagination.totalOrders > 0}
            <p class="text-sm text-[#809BB5]">
              Showing {(currentPage - 1) * data.pagination.limit +
                1}-{(currentPage - 1) * data.pagination.limit +
                data.orders.length} of {data.pagination.totalOrders} orders
            </p>
          {/if}
        </div>
      ...
      <div class="flex flex-col flex-grow min-h-0">
        {#if data.orders && data.orders.length > 0}
          <div class="flex-grow overflow-x-auto">
            <table
              class="w-full border-separate border-spacing-y-2 min-w-[830px]"
            >
              <thead>
                <tr class="text-left">
                ...
                {#each data.orders as order (order._id)}
                  ...
                  >
                    <td class="py-4 pl-4 rounded-l-lg min-w-[160px]">
                      {#if order.robuxPurchase?.robuxAmount}
                        <span class="text-[#809BB5]"> Robux Purchase </span>
                      {:else}
                      ...
                      <div class="flex items-center gap-2">
                        <div class="flex -space-x-2">
                          {#each order.items.slice(0, 3) as item (item._id || Math.random())}
                            <div
                              class="size-8 rounded-full bg-[#131620] ring-2 ring-[#1D2535] relative flex items-center justify-center"
                            >
                              {#if item.image}
                                <img
                                  src={item.image}
                                  alt={item.title ?? 'Item'}
                                  class="object-contain p-1 size-full"
                                />
                              {:else}
                    ...
                    <td>
                      <span
                        class="px-[11px] py-[5px] rounded-full capitalize text-sm font-medium {statusColors[
                          order.status
                        ] ?? 'bg-gray-500'}"
                      >
                        {order.status}
                    ...
                    <td class="pr-4 max-w-[180px]">
                      <Button
                        ...
                        title={order.status === 'pending'
                          ...
                          : 'Roblox account cannot be changed'}
                      ...
                      >
                        {#if order.reciever && order.reciever.displayName}
                          <div class="flex items-center gap-2 pr-2">
                            ...
                            {/if}
                            <span
                              class="text-sm font-medium max-w-[110px] truncate"
                              >{order.reciever.displayName}</span
                            >
                          </div>
                        {:else if order.status === 'pending'}
                          <span class="flex items-center gap-2">
                            <Pencil class="size-4" /> Add reciever</span
                          >
                        {:else}
...
<OrderDetails bind:open={orderDetailsOpen} bind:order={selectedOrder} />

<RobloxAccount bind:open={robloxAccountModalOpen} bind:order={selectedOrder} />

<ClaimModal
  bind:open={claimModalOpen}
  bind:robuxAmount={data.robuxAmount}
  on:claim-success={claimSuccess}
/>
